...
export class UsersStore extends ComponentStore<UsersState> {
  ...
  fetchUsers = this.effect((trigger$) =>
    trigger$.pipe(
      exhaustMap(() => this.usersService.fetchUsers$().pipe(
        tapResponse((users: User[]) => {
          this.setState((state) => adapter.addMany(
            users, state
          ))
        }, (error: HttpErrorResponse) => {
          this.setState((state) => adapter.removeAll({...state, currentUserId: null, error}))
        })
      ))
    )
  )
  ...
  createUser = this.effect((newUser$: Observable<User>) =>
    newUser$.pipe(
      exhaustMap((newUser) => this.usersService.createUser$(newUser).pipe(
        tapResponse((users: User[]) => {
          this.setState((state) => adapter.addMany(
            users, state
          ))
